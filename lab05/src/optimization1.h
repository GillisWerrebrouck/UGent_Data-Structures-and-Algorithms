#ifndef OPTIMIZATION1_H
#define OPTIMIZATION1_H
...
// this method calls the private build method which has one extra parameter, being the index, which is 0 at the start of the algorithm
...
bool is_question_redundant(std::list<std::string> &animals, std::map<std::string, bool> &answers)
{
    int count_yes = 0;
    int count_no = 0;

    // count which animals have true and false
    for(std::string animal : animals) {
        if((*answers.find(animal)).second)
            count_yes++;
        else
            count_no++;
    }

    return (count_yes == 0 || count_no == 0);
}
...
bool BinaryTree::build(std::vector<std::string> &questions, std::list<std::string> &animals, std::vector<std::map<std::string, bool>> &answers, int depth)
{
    ...
    if(depth < questions.size() && is_question_redundant(animals, answers[depth])) {
        // skip this redundant question and build the subtree with the next question
        return this->build(questions, animals, answers, depth+1);
    }
...
double BinaryTree::averageDepth()
{
    ...
    // the index can't be used here to calculate the depth because it isn't always a complete binary 
    // tree anymore as it used to be before optimalisation
